 ...
 * Reverse a PAID payroll period (undo the payment and journal entries)
 */
router.post('/payroll/periods/:id/reverse', async (req, res) => {
  ...
  try {
    ...
    if (payrollPeriod.status !== 'paid') {
      await client.query('ROLLBACK');
      return res.status(400).json({ error: 'Can only reverse paid payroll. Use DELETE for draft periods.' });
    }
    ...
    const entries = await client.query(`
      ...
      WHERE source_type = 'payroll' AND (source_id = $1 OR (source_id = 0 AND description LIKE $2))
    `, [id, '%' + payrollPeriod.period_name + '%']);
    
    // Create reversals
    for (const entry of entries.rows) {
      await client.query(`
        INSERT INTO journal_entries (date, description, account_code, debit, credit, source_type, source_id)
        VALUES (CURRENT_DATE, $1, $2, $3, $4, 'payroll_reversal', $5)
      `, [
        `REVERSA: ${payrollPeriod.period_name}`,
        entry.account_code,
        entry.credit,
        entry.debit,
        id
    ...
    await client.query(`UPDATE payroll_periods SET status = 'reversed', updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [id]);
    await client.query('COMMIT');
    
    console.log(`✅ Reversed payroll ${id} with ${entries.rows.length} entries`);
    res.json({ success: true, entries_reversed: entries.rows.length });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Error reversing payroll:', error);
    res.status(500).json({ error: 'Failed to reverse payroll' });
  } finally {
    client.release();
  }
});
